...
├── assets
│   ├── ai
│   │   └── config.js  # legacy fallback, not loaded by default
│   ├── icons
│   │   └── favicon.svg
│   ├── images
...
6. Wrangler will print a Worker URL. Add `/chat` to it, then paste it into the `<body data-ai-endpoint="...">` attribute in `index.html`:

```html
<body data-ai-endpoint="https://zuriel-portfolio-ai.your-subdomain.workers.dev/chat">
```

7. Commit and push the updated `index.html` to GitHub Pages.
...
The static front end uses no external JavaScript libraries or CDN scripts. This keeps the page compatible with strict CSP settings.
...
## v34 CSP-compatible AI fix

This version removes third-party CDN scripts and inline configuration so it works with a strict Content Security Policy such as `script-src-elem 'self' https://static.cloudflareinsights.com`.

The portfolio now reads the AI Worker URL from the `<body data-ai-endpoint="...">` attribute in `index.html`, and loads only one local JavaScript file: `./script.js`.

When deploying, upload the whole folder, not only `index.html`. The files `script.js` and `styles.css` must be present in the same root folder as `index.html`.
