...
## v34 Ask AI loading fix

This version removes blocked CDN scripts and removes the separate `assets/ai/config.js` dependency. The site now loads one local JavaScript file: `assets/js/app.js`. The Cloudflare Worker endpoint is also built into the script and into the `<body data-ai-endpoint>` attribute.

After deployment, open these URLs directly:

- `https://zurielst.com/assets/js/app.js` should show JavaScript code, not an HTML error page.
- `https://zuriel-ai-resume-assistant.zuriel-shanley.workers.dev/chat?message=hello` should return JSON.

If your Cloudflare dashboard has a custom CSP rule, make sure `script-src-elem` allows `'self'` and `connect-src` allows `https://zuriel-ai-resume-assistant.zuriel-shanley.workers.dev`.

## CSP note for the AI assistant

The AI assistant calls this Cloudflare Worker endpoint:

```text
https://zuriel-ai-resume-assistant.zuriel-shanley.workers.dev/chat
```

If the browser console shows a `connect-src` Content Security Policy error, update the site CSP so `connect-src` includes the Worker origin:

```text
connect-src 'self' https://cloudflareinsights.com https://zuriel-ai-resume-assistant.zuriel-shanley.workers.dev;
```

This package includes a Cloudflare Pages `_headers` file with the correct CSP. If the site is hosted on GitHub Pages behind Cloudflare, `_headers` will not control the response headers; update the CSP from the Cloudflare dashboard instead.
