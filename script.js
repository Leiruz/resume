...
const bodyAIEndpoint = document.body?.dataset?.aiEndpoint || '';
const aiEndpoint = (bodyAIEndpoint || (typeof aiConfig.endpoint === 'string' ? aiConfig.endpoint : '') || DEFAULT_AI_ENDPOINT).trim();
...
function buildAIRunURL(question) {
  // Keep the public AI route narrow and WAF-friendly:
  // no query string, no chat history in the URL, and a short prompt payload.
  const safeQuestion = String(question || '').replace(/\s+/g, ' ').trim().slice(0, 280);
  const payload = toBase64Url(JSON.stringify({ message: safeQuestion }));

  const endpoint = new URL(aiEndpoint, window.location.href);
  const basePath = endpoint.pathname.replace(/\/chat\/?$/i, '').replace(/\/$/, '') || '';
  endpoint.pathname = `${basePath}/run/${payload}`.replace(/\/+/g, '/');
  endpoint.search = '';
...
async function fetchPortfolioAI(question) {
  // Single public path: GET /ai/run/{base64urlPayload}.
  // This avoids query strings and removes the POST fallback attack surface.
  const runResponse = await fetch(buildAIRunURL(question), {
    method: 'GET',
    cache: 'no-store'
  });
  return await readAIResponse(runResponse);
}
...
async function askPortfolioAI(question) {
  ...
  if (!aiEndpoint) {
    appendAIMessage('assistant', 'The AI assistant is not connected yet. Check the Cloudflare Worker route /ai/chat.');
    return;
  }
  ...
  try {
    ...
    if (pending) pending.textContent = 'The AI assistant is unavailable right now. Please refresh the page and try again.';
...
aiPromptButtons.forEach((button) => {
  button.addEventListener('click', () => {
    ...
    if (!aiInput || !prompt) return;
    aiInput.value = prompt;
    aiInput.focus();
    aiInput.setSelectionRange(prompt.length, prompt.length);
  });
});
