...
async function fetchPortfolioAI(question) {
  ...
  // Primary path: send the prompt as a compact encoded path segment. This avoids
  // Cloudflare WAF false positives that can occur when security-related text is
  // placed directly in the query string.
  try {
    ...
    // Fallback path: encoded text/plain body. This keeps the message out of the
    // URL while avoiding a JSON preflight in restrictive browser/CDN setups.
    console.warn('AI assistant encoded route failed; trying text fallback:', runError);
