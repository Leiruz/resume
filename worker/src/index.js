...
export default {
  async fetch(request, env) {
    ...
    // Public diagnostics are intentionally disabled. They were useful during setup,
    // but should not reveal binding, model, origin, or implementation details.
    // Only /ai/run/{encodedPayload} should be reachable from the website.
    if (path === '/' || path === '/info' || path === '/health' || path === '/test' || path === '/cors-check' || path === '/chat') {
    ...
    const payload = parseEncodedPayload(encodedFromPath);
    ...
    try {
      ...
      return json({ answer: result.answer }, 200, request, env);
    } catch (error) {
      console.error('AI assistant error:', error?.message || error);
      return json({ error: 'The AI assistant is unavailable right now.' }, 500, request, env);
    }
  }
};
