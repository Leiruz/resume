...
function parseEncodedPayload(value) {
  ...
  try {
    ...
    return parsed && typeof parsed === 'object' ? parsed : { message: decoded };
  } catch {
...
function payloadFromPath(path) {
  const match = String(path || '').match(/^\/run\/([^/?#]+)$/);
...
function routeKind(path) {
  ...
  if (cleanPath.startsWith('/run/')) return 'chat';
  return 'not-found';
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && path === '/') {
      return json({ ok: true, service: 'Zuriel AI Resume Assistant', endpoint: '/ai/chat', usage: 'GET /ai/run/{encodedPayload}, GET /ai/chat?message=..., or POST /ai/chat. Direct /chat and /api/ai/chat also work.', health: '/ai/health', test: '/ai/test' }, 200, request, env);
    }
    ...
    if (path !== '/chat' && !path.startsWith('/run/')) {
      return json({ error: 'Route not found. Use /ai/run/{encodedPayload} or /ai/chat.' }, 404, request, env);
    }
    ...
    if (request.method === 'GET') {
      // Preferred website path: /ai/run/{base64url(JSON)}. Keeping the prompt out
      // of the query string avoids false positives from security filters that scan
      // cybersecurity terms in URLs.
      const encodedFromPath = payloadFromPath(path);
      const encodedFromQuery = url.searchParams.get('payload') || url.searchParams.get('b64') || '';
      if (encodedFromPath || encodedFromQuery) {
        const payload = parseEncodedPayload(encodedFromPath || encodedFromQuery);
        message = clean(payload.message || payload.question || payload.prompt || payload.input || '');
      ...
      if (contentType.includes('application/json')) {
        ...
        body = parseEncodedPayload(rawText);
      }

      message = clean(body.message || body.question || body.prompt || body.input || '');
      history = Array.isArray(body.history) ? body.history : [];
    } else {
      return json({ error: 'Use GET /ai/run/{encodedPayload} or POST /ai/chat.' }, 405, request, env);
    }
